...
namespace Poliglot
{
    ...
    public class Transformer
    {
        ...
        public Transformer()
        {
            GetterCode = "return Ioc.Get<ITranslator>().Translate(\"{0}\");";
        }
        ...
        public void Transform(string filePath, GenerationType type)
        {
            ...
            try
            {
                Generate(FetchPhrazes(filePath).OrderBy(kvp => kvp.Key).ToDictionary(kp => kp.Key, kp => kp.Value), type);
            }
            catch (Exception ex)
            {
        ...
        private IDictionary<string, string> FetchPhrazes(string filePath)
        {
            ...
            if (ext == ".xml")
            {
                return FetchFromDroidResources(filePath);
            }
            if (ext == ".strings")
            {
        ...
        private IDictionary<string, string> FetchFromResx(string filePath)
        {
            ...
            foreach (XElement element in dataElements)
            {
                ...
                {
                    string cleanedValue = valueElement.Value.Replace("'", "\\'");
        ...
        private IDictionary<string, string> FetchFromDroidResources(string filePath)
        {
            var result = new Dictionary<string, string>();
        ...
        {
            IntWriteCommentLine("Generated by Poliglot.Transformer tool.");
            IntWriteCommentLine(string.Format("getter string is '{0}'", GetterCode));
            IntWriteLine("namespace " + Namespace);
            IntWriteLine("{");

            var root = new LocaleNode { IsClass = true, Name = RootClass };
        ...
        private void GenerateAccessors(StringBuilder builder, LocaleNode node, int indentLevel)
        {
            ...
            else
            {
                ...
                builder.AppendLine(il + "{");
                string getter = string.Format(GetterCode, node.KeyValue);
                builder.AppendFormat("{0}get {{ {1} }}\n", Indent(indentLevel + 1), getter);
                builder.AppendLine(il + "}");
            }
        }
